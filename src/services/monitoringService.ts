...
export class MonitoringService {
   ...
   * Get system alerts
   */
  async getAlerts(options: { acknowledged?: boolean; severity?: string } = {}): Promise<any[]> {
    try {
      const where: any = {};
      ...
      if (options.severity) {
        where.severity = options.severity.toUpperCase();
      }
      ...
      return alerts.map(alert => ({
        ...
        source: alert.server?.name || 'System',
      ...
      // Return mock alerts if database doesn't have alert table
      return [
        {
          id: '1',
          type: 'warning',
  ...
  async acknowledgeAlert(alertId: string, userId: string): Promise<void> {
    try {
      ...
      // Silently fail if alert table doesn't exist
    }
  }
   ...
   */
  async exportData(timeRange: string = '24h', format: 'json' | 'csv' = 'json'): Promise<any> {
    try {
      const timeRanges = {
        '1h': 1,
        '6h': 6,
      ...
      const hoursAgo = timeRanges[timeRange as keyof typeof timeRanges] || 24;
      const startTime = new Date(Date.now() - hoursAgo * 60 * 60 * 1000);
      ...
      // Get system stats
      const systemStats = await this.getAggregatedStats();
      ...
      if (format === 'csv') {
        ...
        const headers = ['timestamp', 'serverId', 'serverName', 'cpu', 'memory', 'disk', 'networkIn', 'networkOut', 'players'];
        const csvData = [
          headers.join(','),
   ...
   */
  async compareServers(serverIds: string[], timeRange: string = '24h', metric: string = 'cpu'): Promise<any> {
    try {
      const timeRanges = {
        '1h': 1,
        '6h': 6,
        ...
        serverIds.map(async (serverId) => {
          ...
          const metricValues = metrics.map(m => {
            switch (metric) {
              ...
              case 'networkIn': return m.networkIn;
              case 'networkOut': return m.networkOut;
              case 'players': return m.players;
              default: return m.cpu;
            }
          });
          ...
          return {
            ...
            values: metricValues,
            average: metricValues.length > 0 ? metricValues.reduce((a, b) => a + b, 0) / metricValues.length : 0,

...
\`\`\``;
  ...
  async install(pluginPath) {
    ...
    try {
      ...
      if (existingPlugin) {
        console.log(`⚠️  Plugin ${metadata.name} already installed`);
        return false;
      }
      ...
      console.log(`✅ Plugin ${metadata.name} installed successfully`);
  ...
  async disable(pluginName) {
    try {
      ...
      if (plugin.status === 'INACTIVE') {
        console.log(`✅ Plugin ${pluginName} is already inactive`);
        return true;
      }
...
async function main() {
  ...
  try {
    switch (command) {
      case 'create':
        if (!args[1]) {
          console.error('❌ Plugin name required: npm run plugin:create <name>');
          process.exit(1);
        }
      ...
      case 'install':
        if (!args[1]) {
          console.error('❌ Plugin path required: npm run plugin:install <path>');
          process.exit(1);
        }
      ...
      case 'enable':
        if (!args[1]) {
          console.error('❌ Plugin name required: npm run plugin:enable <name>');
          process.exit(1);
        }
      ...
      case 'disable':
        if (!args[1]) {
          console.error('❌ Plugin name required: npm run plugin:disable <name>');
          process.exit(1);
        }
